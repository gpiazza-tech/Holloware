...
namespace Holloware
{
	void PythonInterpreter::Begin()
	{
		...
		ExecutePyFilesAt(Application::Get().GetCurrentProject().GetAssetsPath() / "scripts");
	}
	...
	void PythonInterpreter::BindEntityToScript(Entity entity, PythonScriptComponent& psc)
	{
		HW_PROFILE_FUNCTION();
		...
		try
		{
			...
			psc.Instance = new PythonEntity(className.c_str(), entity.GetComponent<TransformComponent>().Position, entity.GetComponent<TransformComponent>().Rotation, entity.GetComponent<TransformComponent>().Scale);
		}
		catch (std::exception e)
		{
			HW_CORE_ERROR("Failed to bind entity {0} to script {1}!", entity.GetComponent<TagComponent>().Tag, psc.ScriptAsset.GetPath().string());
		}
	}

	void PythonInterpreter::FreeEntityFromScript(Entity entity, PythonScriptComponent& psc)
	{
		HW_PROFILE_FUNCTION();
	...
	void PythonInterpreter::ExecutePyFilesAt(const std::filesystem::path& path)
	{
		HW_PROFILE_FUNCTION();
